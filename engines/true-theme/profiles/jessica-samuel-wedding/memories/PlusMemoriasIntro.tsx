...
export function PlusMemoriasIntro({ config, tableId }: PlusMemoriasIntroProps) {
  const tableLabel = getTableLabel(tableId);
  const invitationBacked = config.sourceType === "haxr-invitation";
  const isJessicaSamuel = config.invitationSlug === "jessicasamuelwedding";
  ...
  useEffect(() => {
    setIsPostEvent(
      Boolean(
        config.eventDate &&
        new Date() > new Date(`${config.eventDate}T23:59:59+02:00`)
      )
    );
  }, [config.eventDate]);
  ...
  return (
    <section className="text-center pt-8 pb-10 px-6 max-w-2xl mx-auto">
      ...
      >
        <div className={`relative mx-auto mb-3 ${isJessicaSamuel ? "w-24 h-24 sm:w-28 sm:h-28" : "w-48 h-20 sm:w-56"}`}>
          <Image
            src={isJessicaSamuel ? "/images/jessica-samuel-wedding/monogram.png" : HAXR_AUTH.assets.logoHorizontal}
            alt={isJessicaSamuel ? "Monograma Oficial Jessica & Samuel" : HAXR_AUTH.brand}
            fill
            sizes={isJessicaSamuel ? "112px" : "224px"}
        ...
        <p className="font-body text-[10px] sm:text-[11px] font-light tracking-[0.28em] uppercase text-[#171312]/55 mb-5">
          {config.eventType}{invitationBacked ? " · HAXR SIGNATURE" : " · PLUS MEMORIES"}
        </p>
