  ...
  try {
    ...
    const rows = (await sql`
      SELECT
        ...
        to_regprocedure('public.reserve_edition_gift(text,text,text,text)') IS NOT NULL AS gift_function
    `) as Array<{
      database_name: string;
      rsvp_function: boolean;
      rate_limit_function: boolean;
      gift_function: boolean;
    }>;

    const row = rows[0];
    const ok = Boolean(
      row?.rsvp_function && row?.rate_limit_function && row?.gift_function
    );
    ...
    return NextResponse.json(
      {
        ...
        giftFunction: Boolean(row?.gift_function),
      },
      { status: ok ? 200 : 503 }
    );
  } catch (error) {
