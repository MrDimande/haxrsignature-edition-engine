...
if (!databaseUrl) {
  console.error("[neon-rsvp-canary] DATABASE_URL missing");
  process.exit(2);
}
...
try {
  ...
  console.error(
    `[neon-rsvp-canary] failed=${error instanceof Error ? error.message : "unknown"}`
  );
  process.exitCode = 3;
} finally {
  try {
    ...
    console.error(
      `[neon-rsvp-canary] cleanup_failed=${
        cleanupError instanceof Error ? cleanupError.name : "DatabaseError"
      }`
    );
    process.exitCode = 4;
  }
